...
(function($, window, undefined) {
    ...
    function buildCalendar(classnameContext, options) {
        ...
        function beforeShowDay(date) {
            ...
            if (range.start && ((+date === +range.start) || (range.end && range.start <= date && date <= range.end))) {
                is_highlight = 'ui-state-highlight';
            }
            var result = [
                    true, // selectable
                    is_highlight // class to be added
                ],
                userResult = [true, ''];
    ...
    function buildDateRangePicker($originalElement, options) {
        ...
        function setRange(value, reset) {
            if ($('.' + classname + '-presets .ui-menu').hasClass('active')) {
                $('.' + classname + '-presets .ui-menu').removeClass('active');
            } else {
                $('.' + classname + '-presets .ui-menu .ui-menu-item').removeClass('current-preset');
            }
        ...
        function reposition() {
            $container.position({
                ...
                using: function(coords, feedback) {
                    ...
                    $container.css({
                        left: feedback.element.left,
                        top: feedback.element.top,
                    });
